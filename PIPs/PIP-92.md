...
3. **Custody check.** The exit must be started by, and pays out to, the Ethereum address equal to the Polygon burning address. Control of that address on Ethereum is confirmed before the burn is sent. If the fee-collection account's address is not under matching control on Ethereum, `X` is first moved on Polygon PoS to an account whose Ethereum counterpart is, and the burn is sent from there.
...
- **Bridge and custody.** `X` leaves Polygon PoS by a Plasma burn and exit, and the exit pays POL to the Ethereum address matching the burning account. Control of that address on Ethereum is verified before the burn (a contract at the same address on both networks does not imply the same signers), holding time before the StakeManager transfer is minimised, and all hashes are published.
